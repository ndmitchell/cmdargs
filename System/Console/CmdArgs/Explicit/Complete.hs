...
completeBash prog =
    ...
    ,"_" ++ prog ++ "()"
    ,"{"
    ,"    #local CMDARGS_DEBUG=cmdargs.tmp # uncomment to debug this script"
    ,""
    ,"    COMPREPLY=()"
    ...
    ,"        echo Call \\(${COMP_WORDS[@]:1}, $CMDARGS_COMPLETE\\) > $CMDARGS_DEBUG"
    ,"        echo $result >> $CMDARGS_DEBUG"
    ,"    fi"
    ...
    ,""
    ,"    if [ -n $CMDARGS_DEBUG ]; then"
    ,"        echo echo COMPREPLY: ${#COMPREPLY[@]} = ${COMPREPLY[@]} >> $CMDARGS_DEBUG"
    ,"    fi"
    ,"}"
